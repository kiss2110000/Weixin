...
Element = {
    # 微信版本 6.6.7
    "照片列表": "com.tencent.mm:id/j2",

    # 文字部件
    "纯文字部件": "com.tencent.mm:id/ib",
    "描述文字部件": "com.tencent.mm:id/aad",
    "计数文字部件": "com.tencent.mm:id/diy",

    # 视频部件 图片三连图
    "视频1部件": "com.tencent.mm:id/dep",
    "视频2部件": "com.tencent.mm:id/deq",
    "视频3部件": "com.tencent.mm:id/der",

    # 检测模式 Gallery
    "阅读模式": "com.tencent.mm:id/dhm",

    # 进入详情按钮
    "评论按钮": "com.tencent.mm:id/dh1",

    # 详情
    "详情文字": "com.tencent.mm:id/ib",
    "九宫格": "com.tencent.mm:id/dk2",

    # 上传
    # "拍照分享": "com.tencent.mm:id/jr",
    # "照片池": "com.tencent.mm:id/hp",
...
Element2 = {
    # 微信 7.0.0
    "照片列表": "com.tencent.mm:id/lc",

    # 文字部件
    "纯文字部件": "com.tencent.mm:id/km",
    "描述文字部件": "com.tencent.mm:id/nd",
    "计数文字部件": "com.tencent.mm:id/edu",

    # 视频部件
    "视频1部件": "com.tencent.mm:id/ea5",
    "视频2部件": "com.tencent.mm:id/ea6",
    "视频3部件": "com.tencent.mm:id/ea7",
    # 检测模式
    "阅读模式": "com.tencent.mm:id/ecm",
    # 进入详情按钮
    "评论按钮": "com.tencent.mm:id/ec1",

    # 详情
    "详情文字": "com.tencent.mm:id/km",
    "九宫格": "com.tencent.mm:id/efe",
...
def checkPhotoElemType(elem):
    ...
    for child in elem.iter():
        ...
        if child.attrib["class"] == "android.widget.TextView":
            ...
            # print(Element["描述文字部件"])
            if child.attrib["resource-id"] == Element["纯文字部件"] or \
...
def shoucangElem(elem, elem_type):
    ...
    if elem_type == "word":
        ...
        return True
    elif elem_type == "photoList" or elem_type == "photoWord" or elem_type == "photoWordList":
        for child in elem.iter():
            # 找到每个图片的按钮,点击进入阅读模式,并保存
            if child.attrib["class"] == "android.view.View" and \
                    (child.attrib["resource-id"] == Element["视频1部件"] or
        ...
        return True
    elif elem_type == "video" or elem_type == "videoWord":
        # 找到视频按钮,点击进入阅读模式
        albumJumpReading(elem)
        assert d(className="android.widget.ProgressBar").wait_gone(timeout=600), "下载失败，检查网络是否正常！"
...
def xunhuanPYQ():
    ...
    while result:
        ...
        if elem_type == "empty" or elem_type == "half" or elem_type is None:
            ...
            continue
        # 开始下载保存和上传发表
        result = shoucangElem(last_elem, elem_type)
        if result is False:
            return False
        # 发表完后向下滑动
        num += 1
    ...
    if ui.exists() is True and ui.center()[1] > 700 and result is True:
        ...
        for elem in reversed(elms):
            ...
            if elem_type == "empty" or elem_type is None:
                ...
                continue
            print("检测到的类型:", elem_type)
            result = shoucangElem(elem, elem_type)
            num += 1
            if result is False:
                return False
    ...
    print(" -- 总共转发{}  {}".format(num, str_time))
